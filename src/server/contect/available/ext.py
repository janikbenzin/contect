...
from contect.available.available import AvailableSelections, AvailableNormRanges
...
class ContectExtension:
    """A generic class for extensible objects of contect.
...
- typ: An instance of ContectExtensible.
...
"""
    ...
    resource: Optional[bool]  # Contains true if resoure is needed in the current implementation of a detector
    ...
    _helper_granular: Any  # contains the constructor for the helper with non-global drift (e.g. yearly)
    _log_param: List[str]  # Contains a list of attribute names of the ObjectCentricLog that is used as a param
    ...
    _typ: ContectExtensible
    _callable: Callable  # Used for various purposes (identification, extraction...)
    _callable_helper_global: Callable  # Used only for extraction of helper aggregators with global drift
    _callable_helper_granular: Callable  # Used only for extraction of helper aggregators with non-global drift

    def build_entity(self, time, **kwargs):
        param_args = {arg: kwargs[arg] for arg in kwargs if arg in self._arguments}
        return self._entity(time=time, selection=kwargs['selection'],
                            **{self._extractor_target: self._callable(**param_args)},
                            **self._param)

    def build_helper(self, timespan, param, data, entity, situation, step_width, additional_data, **kwargs):
        return Helper(
            helpers={
                selection:
                    self._helper_global(
                        selection=selection,
                        aggregator=self._callable_helper_global(timespan=timespan,
                                                                param=param,
                                                                data=data[selection],
                                                                entity=entity,
                                                                situation=situation,
                                                                ...
                                                                additional_data=additional_data.add_data_helpers[
                                                                    selection] if
                                                                additional_data is not None else None,
                                                                **kwargs),
                        **self._param_helper
                    ) if param.norm_range is AvailableNormRanges.GLOBAL else
                    self._helper_granular(
                        selection=selection,
                        granularity=param.granularity,
                        aggregator=self._callable_helper_granular(timespan=timespan,
                                                                  param=param,
                                                                  data=data[selection],
                                                                  entity=entity,
                                                                  situation=situation,
                ...
                for selection in self.selections if selection in data
            },
            associated_ce=entity,
            associated_sit=situation
        )

    def build_situation(self, entity, situation, selections, entities,
                        helper, time_to_time_bin, situation_param):
        return Situation({selection:
            {time:
                self._situation(
                    time=time,
                    selection=selection,
                    **{self._extractor_target: self._callable(selection=selection,
                                                              helper=helper,
                                                              time_to_time_bin=time_to_time_bin,
                                                              situation_param=situation_param,
                                                              entity=entities[selection][time],
                                                              time=time)
                       },
                    **self._param)
                for time in entities[selection]
            }
            for selection in selections if selection in situation_param.weights},
            associated_ce=entity,
            associated_sit=situation)
    ...
    def build_param_obj(self, log, **kwargs):
        param_args = {}
        for arg in kwargs:
            param_args = {**param_args, **self.input_display[arg][TRANSFORMATION_KEY](kwargs[arg])}
