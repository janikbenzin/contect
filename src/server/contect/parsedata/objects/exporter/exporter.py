...
from contect.available.available import AvailableSituationType, AvailableSelections, AvailableClassifications
from contect.available.constants import SITUATION_KEY, ANTI_KEY
from contect.parsedata.objects.ocdata import Event, Obj
...
def export_trace_to_dict(trace: Trace, variant=True, detector=None, vmap_params=None, objects=None, threshold=None,
    ...
    color_assignment = get_color_to_events_assignment([event.id for event in trace.events])
    if variant:
        ...
        return {TID_KEY: trace.id,
                EVENTS_KEY:
                    [{EID_KEY: event.id,
                      ...
                      POSITIVE_KEY: round(event.complex_context[detector][AvailableSituationType.POSITIVE], round_n),
                      NEGATIVE_KEY: round(event.complex_context[detector][AvailableSituationType.NEGATIVE], round_n)}
                     for event in trace.events],
                POSITIVE_KEY: round(trace.complex_context[detector][AvailableSituationType.POSITIVE], round_n),
                NEGATIVE_KEY: round(trace.complex_context[detector][AvailableSituationType.NEGATIVE], round_n),
                SCORE_KEY: round(trace.score[detector], round_n),
                CA_SCORE_KEY: round(trace.ca_score[detector], round_n) if detector in trace.ca_score else 'na',
                PS_ALPHA_KEY: get_required_ps(detector, label, round_n, threshold, trace),
                NG_ALPHA_KEY: get_required_ng(detector, label, round_n, threshold, trace)
                }


def get_required_ng(detector, label, round_n, threshold, trace):
    score = trace.score[detector] if detector not in trace.ca_score else trace.ca_score[detector]
    if (label is AvailableClassifications.N or label is AvailableClassifications.CAN) and score < threshold:
        alpha = get_ng_alpha(score, trace.complex_context[detector], threshold)
        if alpha == 'na':
            return alpha
        else:
            return round(alpha, round_n)
    else:
        return 'na'


def get_required_ps(detector, label, round_n, threshold, trace):
    score = trace.score[detector] if detector not in trace.ca_score else trace.ca_score[detector]
    if (label is AvailableClassifications.D or label is AvailableClassifications.CAD) and score > threshold:
        alpha = get_ps_alpha(score, trace.complex_context[detector], threshold)
        if alpha == 'na':
            return alpha
        else:
            return round(alpha, round_n)
...
def export_oc_data_events_to_dataframe(events: Dict[str, Event], objects: Dict[str, Obj], rows=None) -> pd.DataFrame:
    export_dicts, cols = export_events(objects, [events[eid] for eid in events], rows=rows)
...
def export_trace_to_dataframe(trace: Trace, log: ObjectCentricLog, result=False, detector=None) -> pd.DataFrame:
    export_dicts, cols = export_events(log.objects, trace.events, result, detector)
    df = pd.DataFrame(export_dicts).T
    return df[cols]
...
def export_events(objects, events, result=False, detector=None, rows=None):
    if not result:
        ...
        events_dict = {index:
                     ...
                     **{ot.title() + ':Object':
                            ', '.join([obj for obj in event.omap if objects[obj].type == ot])
...
def export_to_pm4py(log: ObjectCentricLog) -> EventLog:
    ...
    parameters = {log_converter.Variants.TO_EVENT_LOG.value.Parameters.CASE_ID_KEY: 'case:' + TID_KEY.title()}
