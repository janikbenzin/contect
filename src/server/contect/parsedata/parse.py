...
from contect.parsedata.config.param import JsonParseParameters, CsvParseParameters

logger = logging.getLogger('contectLogger')
...
def parse_csv(df: ddf.DataFrame, cfg: CsvParseParameters) -> ObjectCentricData:
    ...
    attr_typ = {attr: name_type(str(df.dtypes[attr])) for attr in cfg.val_names}
...
def parse_json(data: Dict[str, Any], cfg: JsonParseParameters) -> ObjectCentricData:
    ...
    attr_types = list({attr_events[v] for v in attr_events}.union({attr_objects[v] for v in attr_objects}))
