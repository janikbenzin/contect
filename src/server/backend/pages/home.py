...
from backend.param.constants import CSV, JSON, DEFAULT_JOBS, CSV_ATTRIBUTES_FST, NA, CSV_ATTRIBUTES_SND, \
    ...
    ATTRIBUTE_CSV_TEXT, ATTRIBUTE_OCEL_TEXT, SHOW_PREVIEW_ROWS, DEV_CTX_TITLE
...
from contect.available.available import AvailableTasks, AvailableSelections
from contect.parsedata.objects.exporter.exporter import export_oc_data_events_to_dataframe
...
goto_title = 'Correlate'
remove_jobs_title = 'remove jobs'
start_from_last_job = 'selected job'
                    ...
                    [
                        dbc.Col(
                            dcc.Upload(
                                id='upload-data',
                                children=html.Div([
                                    'Drag and Drop or ', html.A('Select a CSV or OCEL JSON File')
                                ],
...
jobs_tab_content = card(
    [
        single_row(html.H2("Jobs Dashboard")),
...
# Page layout
page_layout = container("Context-aware detection of deviations in process executions",
                        [
                            dbc.Tabs([
                                dbc.Tab(upload_tab_content, label="Upload", tab_id='upload'),
                                dbc.Tab(jobs_tab_content, label="Jobs", tab_id='jobs')
                            ], id='home-tabs')
                        ])
...
def run_parse_log(n, children, jobs, temp_jobs, form):
    if n is not None:
        activity, location, log_hash, objects, resource, timestamp, values = read_active_attribute_form(children)
        objects = guarantee_list_input(objects)
        values = guarantee_list_input(values)
        values = list({val: 0 for val in values + [resource] + [location] if val != NA}.keys())
        if form is None:
            form = DEFAULT_FORM
        form[log_hash] = get_attribute_form_dict(activity, location, objects, resource, timestamp, values)
        user = request.authorization['username']
        if jobs[JOBS_KEY][log_hash][JOB_DATA_TYPE_KEY] == CSV:
            df = get_remote_data(user, log_hash, jobs, AvailableTasks.UPLOAD.value)
            csv_param = build_csv_param(activity, location, objects, resource, timestamp, values)
            ...
            oc_data = get_remote_data(user, log_hash, jobs, AvailableTasks.UPLOAD.value)
            if oc_data is None:
                return no_update(3)
            loc, res = set_special_attributes(location, resource)
...
def upload_data(session, content, name, jobs, options, last_job):
    if content is not None:
        ...
        if check_existing_job(jobs, log_hash):
            ...
            return dash.no_update, \
                   dash.no_update, \
                   html.Div(dbc.Alert("You have uploaded this log already. Please look for job id "
            ...
            if success:
                if data_format == CSV:
                    run_task(jobs, log_hash, AvailableTasks.UPLOAD.value, store_redis_backend, data=out)
                else:
        ...
        if jobs is not None and len(jobs[JOBS_KEY]) != 0:
            labels = [make_job_option(jobs, log_hash) for log_hash in jobs[JOBS_KEY]]
...
def update_home_output(n, value, radio_value, at, jobs, nonactive, active, init, form):
    ...
    if init and len(jobs[JOBS_KEY]) != 0:
        # Initialize
        dfs = [get_remote_data(user, log_hash, jobs, AvailableTasks.UPLOAD.value) for log_hash in jobs[JOBS_KEY]]
        ...
        return dash.no_update, dash.no_update, forms, dash.no_update, False, \
               write_global_signal_value([radio_value, str(datetime.now())]), dash.no_update

    else:
        rows = SHOW_PREVIEW_ROWS
        if value is not None and at == 'upload':
            # Newly uploaded data --> new job
            session, log_hash, data_format, name, date = read_global_signal_value(value)
            if data_format == CSV:
                data = get_remote_data(user, log_hash, jobs, AvailableTasks.UPLOAD.value)
                return True, \
                       create_data_table(date, data, name, rows), \
                       nonactive + active, \
                       [html.Div(
                           [
                               radio_items([log_hash], log_hash, log_hash, persistence=True, style=NO_DISPLAY),
                               html.Br(),
                ...
                oc_data = get_remote_data(user, log_hash, jobs, AvailableTasks.UPLOAD.value)
                df_events = export_oc_data_events_to_dataframe(oc_data.raw.events, oc_data.raw.objects, rows)
                return True, \
            ...
            if n is not None and nonactive is not None:
                if len(nonactive) == 0:
                    return no_update(7)
                data = get_remote_data(user, radio_value, jobs, AvailableTasks.UPLOAD.value)
                ...
                if isinstance(data, DataFrame):
                    ...
                    df_events = export_oc_data_events_to_dataframe(data.raw.events, data.raw.objects, rows)
                    return True, \
                           create_data_table(jobs[JOBS_KEY][radio_value][JOB_DATA_DATE_KEY],
                                             df_events,
                                             jobs[JOBS_KEY][radio_value][JOB_DATA_NAME_KEY],
...
def init_jobs(n, jobs, jobs1, jobs2, jobs3, jobs4, jobs5, jobs6, jobs7, jobs8, jobs9):
    jobs_list = [jobs, jobs1, jobs2, jobs3, jobs4, jobs5, jobs6, jobs7, jobs8, jobs9]
    if n is not None and any([job is not None for job in jobs_list]):
        ...
        return tuple([dash.no_update] * 18)
