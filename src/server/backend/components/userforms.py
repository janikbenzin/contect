...
from contect.available.available import extract_options, AvailableNormRanges, AvailableSituationType
        ...
        dbc.Row(
            [
                ...
                dbc.Col(
                    [
                        dcc.Dropdown(
                            id=dropdown_id_maker(extract_title(context) + '-' + titles[3]),
                            options=[
                                dict(label=option.value.title(), value=option.value)
                                for option in AvailableSituationType
                            ],
                            persistence=True,
                            persistence_type='memory'
                        )
                    ...
                    dbc.FormGroup(
                        [
                            ...
                            dbc.Input(placeholder=description[PLACEHOLDER_KEY],
                                      type="text",
                                      id=input_id(extract_title(detector) + SEP + param),
                                      persistence=True,
                                      persistence_type='memory'
                                      ),
    ...
    return [
        html.Div(
            [
                ...
                form_dropdown_row(
                    [NA] + [col for col in df.columns] if isinstance(df, DataFrame) else [NA] + df.meta.attr_events,
                    CSV_ATTRIBUTES_SND,
                    [False, False],
                    log_hash,
                    True,
                    persisted=form[log_hash] if form is not None and log_hash in form and OBJECTS.title() in form[log_hash] else None,
                    list=[])
