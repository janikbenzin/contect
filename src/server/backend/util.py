...
import pandas as pd
from backend.param.available import AvailableSituationsExt
from backend.param.constants import JOB_ID_KEY, JOBS_KEY, JOB_DATA_TYPE_KEY, JOB_DATA_NAME_KEY, JOB_DATA_DATE_KEY, \
    JOB_TASKS_KEY, SEP, NA, CSV, PROPS, CHILDREN, VALUE, OBJECTS, TIMESTAMP, VALUES, CORR_METHOD
from backend.tasks.tasks import celery, get_task, db, results_key
from celery.result import AsyncResult
from contect.available.available import AvailableSelections, AvailableTasks
from contect.available.constants import INTERVALS
...
def check_most_recent_task(jobs1, jobs2, log_hash):
    ...
    try:
        date1 = datetime.strptime(jobs1[JOBS_KEY][log_hash][JOB_TASKS_KEY][JOB_DATA_DATE_KEY], '%Y-%m-%d %H:%M:%S.%f')
    except (KeyError, TypeError) as e:
        # The first is not existing -> the second is newer
        return True
    try:
        date2 = datetime.strptime(jobs2[JOBS_KEY][log_hash][JOB_TASKS_KEY][JOB_DATA_DATE_KEY], '%Y-%m-%d %H:%M:%S.%f')
    except (KeyError, TypeError) as e:
...
def run_task(jobs, log_hash, task_type, task, temp_jobs=None, **kwargs):
    ...
    jobs[JOBS_KEY][log_hash][JOB_TASKS_KEY][JOB_DATA_DATE_KEY] = str(datetime.now())
    return task.id
...
def parse_contents(content, data_format):
    ...
    print('Computing value with {}'.format(data_format))
    try:
        if CSV == data_format:
...
def get_attribute_form_dict(activity, location, objects, resource, timestamp, values):
    return {AvailableSelections.ACTIVITY.value.title(): activity,
            AvailableSelections.LOCATION.value.title(): location,
            OBJECTS.title(): SEP.join(objects),
            AvailableSelections.RESOURCE.value.title(): resource,
...
def build_csv_param(activity, location, objects, resource, timestamp, values):
    ...
    vmap_availables = {}
    if location != NA:
        vmap_availables[AvailableSelections.LOCATION] = True
        vmap_params[AvailableSelections.LOCATION] = location
    else:
        vmap_availables[AvailableSelections.LOCATION] = False
    if resource != NA:
        vmap_availables[AvailableSelections.RESOURCE] = True
        vmap_params[AvailableSelections.RESOURCE] = resource
    else:
        vmap_availables[AvailableSelections.RESOURCE] = False
...
def read_dev_ctx_form_dict(form, log_hash):
    args = []
    for index, entry in form[log_hash].items():
        if int(index) < 3 * len(AvailableSituationsExt):
            args.append([] if int(entry) == 0 else [1])
        else:
            args.append(entry)
    return tuple(args)
